...
export function setupSocketHandlers(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    // CREATE SESSION
    // ========================================
    socket.on(CLIENT_EVENTS.CREATE_SESSION, (payload: CreateSessionPayload) => {
      try {
        const { sessionName, scrumMasterName, pointScale, sessionType } = payload;

        const { session, participantId } = SessionStore.createSession(
          sessionName,
          scrumMasterName,
          pointScale,
          sessionType
    ...
    // JOIN SESSION
    // ========================================
    socket.on(CLIENT_EVENTS.JOIN_SESSION, (payload: JoinSessionPayload) => {
      try {
        ...
        const newParticipant = result.session.participants.find(
    ...
    // ========================================
    socket.on(CLIENT_EVENTS.RECONNECT, (payload: { sessionId: string; participantId: string }) => {
      try {
    ...
    // ADD STORY
    // ========================================
    socket.on(CLIENT_EVENTS.ADD_STORY, (payload: AddStoryPayload) => {
    ...
    // REORDER STORIES
    // ========================================
    socket.on(CLIENT_EVENTS.REORDER_STORIES, (payload: ReorderStoriesPayload) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
    ...
    // START VOTING
    // ========================================
    socket.on(CLIENT_EVENTS.START_VOTING, (payload: StartVotingPayload) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
    ...
    // SUBMIT VOTE
    // ========================================
    socket.on(CLIENT_EVENTS.SUBMIT_VOTE, (payload: SubmitVotePayload) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
    ...
    // SET FINAL POINTS
    // ========================================
    socket.on(CLIENT_EVENTS.SET_FINAL_POINTS, (payload: SetFinalPointsPayload) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
    ...
    // SET TIMER
    // ========================================
    socket.on(CLIENT_EVENTS.SET_TIMER, (payload: SetTimerPayload) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
