...
export const SessionStore = {
    ...
    setParticipantConnected(sessionId, participantId, isConnected) {
        const session = sessions.get(sessionId);
        if (!session)
            return false;
    ...
    startVoting(sessionId, storyId) {
        ...
        if (session.timerDuration) {
            session.timerEndTime = new Date(Date.now() + session.timerDuration * 1000).toISOString();
        }
        console.log(`[SessionStore] Started voting on "${story.title}" in session ${session.code}`);
        return true;
    },
    // Submit a vote
    submitVote(sessionId, storyId, participantId, value) {
        const session = sessions.get(sessionId);
        if (!session)
            return false;
        const story = session.stories.find((s) => s.id === storyId);
    ...
    setFinalPoints(sessionId, storyId, points) {
        const session = sessions.get(sessionId);
        if (!session)
            return false;
        const story = session.stories.find((s) => s.id === storyId);
